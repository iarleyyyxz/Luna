...
bool Renderer2D::Init(float screenWidth, float screenHeight) {
    ...
    // Corrigindo a inicialização do lineShader
    lineShader = new Shader("Resources/Shaders/line.vs", "Resources/Shaders/line.fs"); // Certifique-se que o VS e FS para linha são diferentes
    if (!lineShader->ID) {
...
void Renderer2D::Shutdown() {
    ...
    if (lineShader) { // Liberar o lineShader também
        delete lineShader;
        lineShader = nullptr;
    }
...
void Renderer2D::DrawLine(glm::vec2 p1, glm::vec2 p2, glm::vec4 color, float thickness) {
    // ATENÇÃO: Para realmente desenhar linhas com espessura > 1, você precisaria de um geometry shader
    // que expanda a linha para um quad, ou desenhar um quad fino diretamente no CPU.
    // Esta implementação utiliza o pipeline GL_LINES, que pode ser limitado pela GPU para espessura.
    // A variável 'thickness' é passada para o shader como um uniform.
    // SEU SHADER DE LINHA PRECISA DE UM UNIFORM 'u_Thickness' E LIDAR COM ELE.

    lineShader->use();
    // Passe a matriz de projeção para o shader de linha
    lineShader->setMat4("u_Projection", projectionMatrix);
    lineShader->setVec4("u_Color", color); // A cor pode vir do uniforme ou do próprio vértice
    lineShader->setFloat("u_Thickness", thickness); // Passe a espessura para o shader de linha

    glBindVertexArray(lineVAO);
    glBindBuffer(GL_ARRAY_BUFFER, lineVBO);

    // Dados dos vértices da linha
...
void Renderer2D::DrawFilledRect(glm::vec2 center, glm::vec2 size, glm::vec4 color, float rotationRadians) {
    // Para desenhar um retângulo preenchido rotacionado, a melhor abordagem é
    // construir uma matriz de transformação (modelo) e passá-la para o shader.
    // Em seguida, use o método drawQuad que aceita uma matriz de transformação.

    // 1. Criar a matriz de modelo: Escala -> Rotação -> Translação
    glm::mat4 model = glm::mat4(1.0f);

    // Translação (move o objeto para o centro do retângulo)
    model = glm::translate(model, glm::vec3(center.x, center.y, 0.0f));

    // Rotação (em torno do eixo Z para 2D)
    ...
    // Agora, chame o seu método drawQuad que aceita uma matriz de transformação.
    // Usaremos a textura branca padrão, já que é um retângulo de cor sólida.
